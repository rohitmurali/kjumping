...
class Game extends Observable {
            ...
            if (gameInProgress()) {
                try {
                    if (_board.whoseMove().equals(RED)) {
                        getPlayer(RED).makeMove();
                    } else {
                        getPlayer(BLUE).makeMove();
                    }
                    checkForWin();
    ...
    private void checkForWin() {
        int sq = _board.size() * _board.size();
        int numBlue = _board.numOfSide(BLUE);
        int numRed = _board.numOfSide(RED);
        if (_playing && (numBlue == sq || numRed == sq)) {
            announceWinner();
            _playing = false;
        }
    }
    ...
    private void readExecuteCommand() {
        try {
            if (!_inp.hasNext("\n")) {
                if (_inp.hasNextInt()) {
                    if (gameInProgress()) {
                        ...
                        throw new GameException("No game in progress");
                    }
                } else {
            ...
            _err.println("Error: syntax error in '<move>' command");
        } catch (GameException ex) {
            reportError(ex.getMessage());
        }
        if (_inp.hasNextLine()) {
